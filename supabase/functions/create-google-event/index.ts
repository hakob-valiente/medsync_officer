...
serve(async (req) => {
  ...
  try {
    ...
    const response = await calendar.events.insert({
      ...
      requestBody: eventData, // In npm:googleapis v3, resource is often requestBody
    });
    ...
    const message = error.errors ? error.errors[0].message : (error.message || "Failed to create event");
