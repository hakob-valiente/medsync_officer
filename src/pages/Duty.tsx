            ...
            <div className="rounded-2xl shadow-2xl w-full max-w-md fade-in" style={{ background: 'var(--card-bg)' }} onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between p-5" style={{ borderBottom: '1px solid var(--border)' }}>
                    <div>
                        ...
                        <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>{officer.fullName} · {officer.studentId}</p>
                    </div>
                ...
                <div className="p-5 space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--text-muted)' }}>Duty Start</label>
                        ...
                        <div>
                            <label className="block text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--text-muted)' }}>Duty End</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text-primary)' }}>Assigned Campus</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text-primary)' }}>Assigned Clinic</label>
                            <div className="relative">
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-3">
                            <label className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>Module Permissions</label>
                            <button
                                type="button"
                                onClick={(e) => { e.preventDefault(); setPerms(perms.length === 4 ? [] : ['appointments', 'medical requests', 'inquiries', 'inventory']); }}
                                className="text-[10px] font-medium px-2 py-1 rounded-md uppercase tracking-wider transition-colors"
                        ...
                        <div className="space-y-3">
                            {['appointments', 'medical requests', 'inquiries', 'inventory'].map((key) => (
                                ...
                                >
                                    ...
                                    <span className="text-sm font-normal capitalize" style={{ color: 'var(--text-primary)' }}>
                                        {key}
                                    </span>
                                </label>
                            ))}
                ...
                <div className="flex gap-3 px-5 pb-5">
                    <button onClick={() => setShowConfirm(true)} disabled={saving}
                        className="btn-cta flex-1 py-2.5 rounded-xl text-sm font-medium text-white disabled:opacity-50"
                    >
                        Save Configuration
                    </button>
                </div>
                ...
                <div className="flex items-center justify-between p-5" style={{ borderBottom: '1px solid var(--border)' }}>
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-medium uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Recent actions across all modules</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style={{ color: 'var(--text-muted)' }}
                        onMouseOver={e => { e.currentTarget.style.background = 'var(--bg-wash)'; }}
                    ...
                    <table className="w-full zebra-table">
                        <thead>
                            <tr style={{ background: 'var(--bg-wash)', borderBottom: '1px solid var(--border)' }}>
                                <th className="text-left text-[10px] font-semibold uppercase tracking-wider px-6 py-3" style={{ color: 'var(--text-muted)' }}>Officer</th>
                                <th className="text-left text-[10px] font-semibold uppercase tracking-wider px-6 py-3" style={{ color: 'var(--text-muted)' }}>Action</th>
                                <th className="text-right text-[10px] font-semibold uppercase tracking-wider px-6 py-3" style={{ color: 'var(--text-muted)' }}>Timestamp</th>
                            </tr>
                        </thead>
                        <tbody>
                            {mockLogs.map((log) => (
                                <tr key={log.id} className="transition-colors" style={{ borderBottom: '1px solid var(--border-light)' }}>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-medium italic"
                                                style={{ background: 'var(--bg-wash)', color: 'var(--text-muted)' }}>
                                                {log.officer.split(' ').map(n=>n[0]).join('')}
                                            </div>
                                            <span className="text-xs font-medium" style={{ color: 'var(--text-primary)' }}>{log.officer}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="text-xs font-normal" style={{ color: 'var(--text-secondary)' }}>{log.action}</span>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex flex-col items-end">
                                            <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>{log.date}</span>
                                            <span className="text-[10px] font-normal" style={{ color: 'var(--text-faint)' }}>{log.time}</span>
                                        </div>
                                    </td>
                                </tr>
                            ))}
            ...
            <div className="rounded-2xl shadow-2xl w-full max-w-md fade-in" style={{ background: 'var(--card-bg)' }} onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between p-5" style={{ borderBottom: '1px solid var(--border)' }}>
                    <div>
                        <div className="flex items-center justify-between mb-1">
                            ...
                            <span className="text-[10px] font-semibold px-2 py-1 rounded-lg ml-3"
                                style={{ color: 'var(--accent)', background: 'var(--accent-light)' }}>
                                {new Date().toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })}
                            </span>
                        </div>
                        <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{user.fullName} · {user.studentId}</p>
                    </div>
                    <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style={{ color: 'var(--text-muted)' }}
                        onMouseOver={e => { e.currentTarget.style.background = 'var(--bg-wash)'; }}
                        onMouseOut={e => { e.currentTarget.style.background = 'transparent'; }}
                ...
                <div className="p-5 space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="col-span-2">
                             <label className="block text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--text-muted)' }}>Assigned Date</label>
                             <input 
                                type="date" 
                                value={assignedDate} 
                             ...
                             {isPast5PM && assignedDate === todayStr && (
                                 <p className="text-[9px] text-red-500 mt-1 font-medium">Duty day ended (past 5 PM). Please select tomorrow or later.</p>
                             )}
                        </div>
                        <div>
                            <label className="block text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--text-muted)' }}>Duty Start</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text-primary)' }}>Assign to Campus</label>
                            <div className="relative">
                                <select value={campus} onChange={(e) => setCampus(e.target.value)}
                                    className="w-full appearance-none rounded-xl px-4 py-2.5 pr-10 text-sm outline-none"
                                    style={{ border: '1px solid var(--border)', background: 'var(--bg-wash)', color: 'var(--text-primary)' }}
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text-primary)' }}>Assign to Clinic</label>
                            <div className="relative">
                                <select value={clinic} onChange={(e) => setClinic(e.target.value)} disabled={!campus}
                                    className="w-full appearance-none rounded-xl px-4 py-2.5 pr-10 text-sm outline-none disabled:opacity-50"
                                    style={{ border: '1px solid var(--border)', background: 'var(--bg-wash)', color: 'var(--text-primary)' }}
                ...
                <div className="flex gap-3 px-5 pb-5">
                    <button
                        ...
                        className="flex-1 py-3 rounded-xl text-[11px] font-bold uppercase tracking-[0.1em] transition-all active:scale-95 disabled:opacity-40"
            ...
            <div className="rounded-2xl shadow-2xl w-full max-w-2xl fade-in flex flex-col max-h-[90vh]" style={{ background: 'var(--card-bg)' }} onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between p-6" style={{ borderBottom: '1px solid var(--border)' }}>
                    <div>
                        ...
                        <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>Found {students.length} matching candidates. Configure their shared duty settings below.</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                             <label className="block text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: 'var(--text-muted)' }}>Assigned Date</label>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                <label className="block text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: 'var(--text-muted)' }}>Start</label>
                                <input type="time" value={dutyStart} onChange={(e) => setDutyStart(e.target.value)}
                            ...
                            <div>
                                <label className="block text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: 'var(--text-muted)' }}>End</label>
                                <input type="time" value={dutyEnd} onChange={(e) => setDutyEnd(e.target.value)}
                                    className="w-full rounded-xl px-3 py-2.5 text-sm outline-none border"
                                    style={{ background: 'var(--bg-wash)', color: 'var(--text-primary)', borderColor: 'var(--border)' }}
                                />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                             <label className="block text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: 'var(--text-muted)' }}>Campus</label>
                             <select value={campus} onChange={(e) => setCampus(e.target.value)}
                        ...
                        <div>
                             <label className="block text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: 'var(--text-muted)' }}>Clinic</label>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Candidate List</h3>
                            <button onClick={toggleAll} className="text-[10px] font-bold uppercase tracking-widest text-emerald-500 hover:opacity-70">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {students.map((student, idx) => (
                                ...
                                >
                                    ...
                                    <div>
                                        <p className="text-sm font-semibold tracking-tight" style={{ color: 'var(--text-primary)' }}>{student.fullName}</p>
                                        <p className="text-[10px] font-bold tracking-widest uppercase" style={{ color: 'var(--text-muted)' }}>{student.studentId}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="p-6 border-t flex gap-3" style={{ background: 'var(--bg-wash)', borderTop: '1px solid var(--border)' }}>
                    <button 
                        ...
                        className="btn-cta flex-1 py-4 rounded-2xl text-xs font-bold uppercase tracking-widest transition-all active:scale-95 disabled:opacity-40"
                    >
    ...
    return (
        <div className="space-y-4">
            <div>
                ...
                <p className="text-sm mt-1 font-medium opacity-70" style={{ color: 'var(--text-secondary)' }}>Assign Officer roles and configure module permissions.</p>
            </div>
            ...
            <div className="rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4 transition-colors" style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl border" style={{ background: 'var(--bg-wash)', borderColor: 'var(--border)' }}>
                        <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Pick</span>
                        <input 
                            type="number" 
                            min={1} 
                            max={50}
                        ...
                        <button 
                            ...
                            className="px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all active:scale-95"
                    ...
                    <button 
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest transition-all shadow-sm"
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-4 scrollbar-hide">
                <button 
                    ...
                    className="px-5 py-2.5 rounded-2xl text-[10px] font-bold uppercase tracking-widest transition-all shadow-sm border whitespace-nowrap"
            ...
            {searchResults.length > 0 && (
                <div className="space-y-2 fade-in p-4 rounded-2xl shadow-sm transition-colors" style={{ background: 'var(--card-bg)', border: '1px solid var(--border)' }}>
                    <h4 className="text-[10px] font-bold uppercase tracking-widest mb-3 ml-1" style={{ color: 'var(--text-faint)' }}>Search Results</h4>
                    {searchResults.map((u) => (
                        <div
                            key={u.id}
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{u.fullName}</span>
                                    <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>{u.studentId}</span>
                                </div>
                            </div>
                            <button
                                onClick={() => setAssignModal(u)}
                                className="flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all active:scale-95"
            ...
            {search.trim().length >= 2 && searchResults.length === 0 && (
                <div className="p-6 rounded-2xl shadow-sm text-center transition-colors" style={{ background: 'var(--card-bg)', border: '1px solid var(--border)' }}>
                    <p className="text-sm font-medium" style={{ color: 'var(--text-faint)' }}>No eligible users found for "{search}"</p>
                </div>
            )}
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
                <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <span className="text-xs" style={{ color: 'var(--text-muted)' }}>{state.officers.length} total</span>
                </div>
                <table className="w-full zebra-table">
                    <thead>
                        <tr style={{ background: 'var(--bg-wash)', borderBottom: '1px solid var(--border)' }}>
                            {['Officer Info', 'Location', 'Duty Status', 'Permissions', 'Actions'].map((h) => (
                                <th key={h} className="text-left text-[10px] font-bold uppercase tracking-widest px-6 py-4" style={{ color: 'var(--text-muted)' }}>
                                    {h}
                                </th>
                            ))}
                        </tr>
                    ...
                    <tbody>
                        {paginatedOfficers.length === 0 ? (
                            <tr>
                                <td colSpan={5} className="text-center text-sm py-20 italic font-medium" style={{ color: 'var(--text-faint)' }}>No officers assigned yet.</td>
                            </tr>
                        ) : (
                            paginatedOfficers.map((officer) => (
                                <tr key={officer.id} className="transition-colors group text-sm" style={{ borderBottom: '1px solid var(--border-light)' }}>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-9 h-9 rounded-xl flex items-center justify-center text-xs font-bold shadow-sm"
                                            ...
                                            <div className="flex flex-col">
                                                <span className="font-semibold text-sm" style={{ color: 'var(--text-primary)' }}>{officer.fullName}</span>
                                                <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>ID: {officer.studentId}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col">
                                            <p className="text-xs font-semibold" style={{ color: 'var(--text-primary)' }}>{state.campuses.find(c => c.id === officer.campus)?.name || 'Unassigned'}</p>
                                            <p className="text-[10px] font-medium" style={{ color: 'var(--text-muted)' }}>{state.clinics.find(c => c.id === officer.clinic)?.name || 'No Clinic'}</p>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col gap-1 items-start">
                                            ...
                                            {officer.assignedAt && <span className="text-[9px] font-bold pl-1 uppercase" style={{ color: 'var(--text-faint)' }}>Since {new Date(officer.assignedAt).toLocaleDateString()}</span>}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex gap-1.5 flex-wrap">
                                            {officer.permissions.map((k) => (
                                                <span key={k} className="text-[10px] px-2.5 py-1 rounded-lg capitalize font-bold tracking-tight shadow-xs"
                                                    style={{ background: 'var(--success-bg)', color: 'var(--success)', border: '1px solid rgba(52,211,153,0.1)' }}>
                                                    {k}
                                                </span>
                                            ))}
                                            {officer.permissions.length === 0 && (
                                                <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-faint)' }}>None</span>
                                            )}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex gap-2 justify-end">
                                            <button
                                                ...
                                                className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest px-3 py-2 rounded-lg transition-all"
